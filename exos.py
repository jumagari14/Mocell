...
##################################
FVA_result=flux_analysis.variability.flux_variability_analysis(model_simple2,fraction_of_optimum=1.0)
print (FVA_result) ## Essentiels (min>0 et max>0 ou l'inverse): Input, Biomass, Expre_biomassreac1=Reaction("Reaction1")
##################################
...
reac1.gene_reaction_rule='gene1'
reac2.gene_reaction_rule='gene2'
reac3.gene_reaction_rule='(gene2 or gene3)'
reac4.gene_reaction_rule='(gene2 or gene4)'

model_KO=Model("KO genes")
model_KO.add_reactions([reac1,reac2,reac3,reac4,r_imp,r_bio,r_exp_bio,r_im_A])

model_KO.objective="Expression_bio"
model_KO.genes.gene2.knock_out()

solution=model_KO.optimize()
print("\nKO GENES\n")
for reac in model_KO.reactions: 
    print(str(reac.id) + " : " + str(solution.fluxes[reac.id]))
##################################
